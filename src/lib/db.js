...
export const toProject = (row) => ({
  ...
  hosting:         Array.isArray(row.hosting)   ? row.hosting   : (row.hosting   ? [row.hosting]   : []),
  database:        Array.isArray(row.database)  ? row.database  : (row.database  ? [row.database]  : []),
  ...
  hasBackend:          row.has_backend          ?? null,
  targetUsers:         row.target_users         || null,
  hasDatabase:         row.has_database         ?? null,
  ...
  builderTools:                   row.builder_tools                     || [],
  hostingAccount:                 row.hosting_account                   || null,
  versionControl:                 row.version_control                   || [],
  versionControlAccount:          row.version_control_account           || null,
  databaseAccount:                row.database_account                  || null,
  screenshotUrls:                 row.screenshot_urls                   || [],
  productionHosting:              row.production_hosting                || null,
  productionHostingUrl:           row.production_hosting_url            || null,
  productionHostingAccount:       row.production_hosting_account        || null,
  productionVersionControl:       row.production_version_control        || null,
  productionVersionControlUrl:    row.production_version_control_url    || null,
  productionVersionControlAccount:row.production_version_control_account|| null,
  productionDatabase:             row.production_database               || null,
  productionDatabaseUrl:          row.production_database_url           || null,
  productionDatabaseAccount:      row.production_database_account       || null,
...
export const fromProject = (proj) => ({
  ...
  problem:          proj.problem      || null,
  built:            proj.built        || null,
  better_now:       proj.betterNow    || null,
  ...
  has_backend:          proj.hasBackend          ?? null,
  target_users:         proj.targetUsers         || null,
  has_database:         proj.hasDatabase         ?? null,
  connects_sprout_db:   proj.connectsSproutDb    ?? null,
  auth_type:            proj.authType?.length > 0 ? proj.authType : null,
  data_sensitivity:     proj.dataSensitivity     || null,
  ...
  hosting:          proj.hosting?.length  > 0 ? proj.hosting  : null,
  database:         proj.database?.length > 0 ? proj.database : null,
  prototype_link:   proj.prototypeLink  ?? null,
  deck_link:        proj.deckLink       ?? null,
  docs_link:        proj.docsLink       ?? null,
  approver_name:             proj.approverName             || null,
  approver_email:            proj.approverEmail            || null,
  approval_status:           proj.approvalStatus           ?? null,
  approval_requested_at:     proj.approvalRequestedAt      ?? null,
  approved_at:               proj.approvedAt               ?? null,
  approval_rejected_at:      proj.approvalRejectedAt       ?? null,
  approval_rejection_reason: proj.approvalRejectionReason  ?? null,
  // edit form v2
  ai_assistant:                    proj.aiAssistant?.length > 0                    ? proj.aiAssistant                    : null,
  builder_tools:                   proj.builderTools?.length > 0                   ? proj.builderTools                   : null,
  hosting_account:                 proj.hostingAccount                             ?? null,
  version_control:                 proj.versionControl?.length > 0                 ? proj.versionControl                 : null,
  version_control_account:         proj.versionControlAccount                      ?? null,
  database_account:                proj.databaseAccount                            ?? null,
  screenshot_urls:                 proj.screenshotUrls?.length > 0                 ? proj.screenshotUrls                 : null,
  production_hosting:              proj.productionHosting                          ?? null,
  production_hosting_url:          proj.productionHostingUrl                       ?? null,
  production_hosting_account:      proj.productionHostingAccount                   ?? null,
  production_version_control:      proj.productionVersionControl                   ?? null,
  production_version_control_url:  proj.productionVersionControlUrl                ?? null,
  production_version_control_account: proj.productionVersionControlAccount         ?? null,
